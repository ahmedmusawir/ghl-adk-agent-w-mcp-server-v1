...
**AI/AGENT DISPLAY RULE:** When displaying monetary values from opportunity responses:
- Always show with 2 decimal places (e.g., 50000 → 50000.00)
- Append currency code with space (e.g., "50000.00 USD")
- Example: monetaryValue: 1500 → display as "1500.00 USD"
...
- monetaryValue: Deal value as float (e.g., 50000.00 for $50,000)
...
⚠️ CRITICAL - MONETARY VALUE FORMAT:
Amount must be a FLOAT with explicit decimal places. The API does NOT use cents.
- For $250.50, use: 250.50 (NOT 25050)
- For $10,000, use: 10000.00
...
- Change opportunity name`,
        inputSchema: {
          ...
          monetaryValue: z.number().optional().describe('Updated monetary value as float with decimals (e.g., 250.50 = $250.50). NOT cents.'),
...
⚠️ CRITICAL - MONETARY VALUE FORMAT:
Amount must be a FLOAT with explicit decimal places. The API does NOT use cents.
- For $500.00, use: 500.00 (NOT 50000)
- For $25,000, use: 25000.00
...
- Idempotent opportunity management`,
        inputSchema: {
          ...
          monetaryValue: z.number().optional().describe('Monetary value as float with decimals (e.g., 500.00 = $500.00). NOT cents.'),
          assignedTo: z.string().optional().describe('User ID to assign this opportunity to')
        }
      },
      {
